...
q = lambda x, y: 24 * x ** 2 * (x - 1) ** 2 + 24 * y ** 2 * (y - 1) ** 2 + 2 * (12 * x ** 2 - 12 * x + 2) * (12 * y ** 2 - 12 * y + 2)
Fiq=[getXY(5,5),q]

def GalerkinPrep(triangles):
    all_mattr_g = []
    for triangle in triangles:
        galer = Galerkin(triangle,Fiq,NM=[50,50])
        galer = galer.do()[0]
        all_mattr_g.append(galer)
    return all_mattr_g
